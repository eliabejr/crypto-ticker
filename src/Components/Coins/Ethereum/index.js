  ...
  render() {
    return (
      <div className="Ethereum-page">
      	...
      	<Helmet>
	      <title>Ethereum | Crypto Watcher</title>
	    </Helmet>

        <NavBar />
		<section className="container" id="exchanges">

			<h2 className="title text-center">Ethereum Price History</h2>
	...
	componentDidMount() {
		...
		const apiEthereumPriceList = 'https://min-api.cryptocompare.com/data/histoday?fsym=ETH&tsym=USD&limit=11&aggregate=3';
		fetch(apiEthereumPriceList)
		.then((resp) => resp.json())
		.then(function(data) {
