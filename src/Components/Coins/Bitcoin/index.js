  ...
  render() {
    return (
      <div className="Bitcoin-page">
      	...
      	<Helmet>
	      <title>Bitcoin | Crypto Watcher</title>
	    </Helmet>

        <NavBar />
		<section className="container" id="exchanges">

			<h2 className="title text-center">Bitcoin Price History</h2>
	...
	componentDidMount() {
		...
		const apiBitcoinPriceList = 'https://min-api.cryptocompare.com/data/histoday?fsym=BTC&tsym=USD&limit=11&aggregate=3';
		fetch(apiBitcoinPriceList)
		.then((resp) => resp.json())
		.then(function(data) {
